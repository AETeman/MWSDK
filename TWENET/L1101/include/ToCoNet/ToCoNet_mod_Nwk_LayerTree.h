...
typedef struct
{
	...
	uint8 u8Second_To_Beacon; //!< 定期的なビーコン情報。旧い接続情報を持つ子機に対して情報を与える (最小値は1)
	...
	uint8 u8LayerOptions; //!< 接続オプション (bit0: 1なら一つ上の階層に限定して接続する)
	uint8 u8minLQI; //!< 接続時の LQI の閾値(これ以上ないと接続しない)

	uint8 u8ResumeOpt; //!< 0x01: レジューム時に RELOCATE しない。
	uint8 u8StartOpt; //!< 0x01: Start時に u32AddrHigherLayer を参照し、SCANしない。 0x02: 上位接続済みノードに接続する。
 ...
 *  必然的に、起動時のスキャンは省略される
 */
#define TOCONET_MOD_LAYERTREE_STARTOPT_CONNECT_READY_NODE 0x02
